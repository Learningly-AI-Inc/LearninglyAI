...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Starting full subscription sync...')
    ...
    for (const user of users?.users || []) {
      try {
        const result = await syncUserWithStripe(admin, stripe, user)
        results.push(result)
        syncedCount++
      } catch (error) {
    ...
    return NextResponse.json({
      success: true,
      message: `Full sync completed. Synced: ${syncedCount}, Errors: ${errorCount}`,
...
async function syncUserWithStripe(admin: any, stripe: Stripe, user: any) {
  try {
    ...
    if (stripeCustomerId) {
      ...
      if (chosenSub) {
        ...
        return {
          ...
          status: 'synced',
          stripeCustomerId,
          stripeSubscriptionId: chosenSub.id,
          planName,
